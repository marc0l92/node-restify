...
function Server(options) {
    ...
    this.preChain = connect();
    this.useChain = connect();
    ...
    this.router._afterRouteLookup = self._afterRouteLookup.bind(this);
...
Server.prototype.getDebugInfo = function getDebugInfo() {
    ...
    if (!self._debugInfo) {
        ...
        var preHandlers = connect
            .extractHandlers(self.preChain)
            .map(funcNameMapper);
        var useHandlers = connect
            .extractHandlers(self.useChain)
            .map(funcNameMapper);
...
//   2. _afterPre (runs after pre handlers are finisehd, triggers use)
//     3. _afterUse (runs after use handlers are finised, triggers route)
//       4. _afterRoute (runs after route handler finished)
//        4.1 _afterRoute (when next('string') called)
//        4.1 _routeErrorResponse (when error happaned in router)
 ...
 * @memberof Server
 * @instance
 * @function _dtraceStartHandler
 * @param    {String} name - handlerName
 * @param    {Request}    req - the request object
 * @returns  {undefined} no return value
 * @fires Request,Response#request
 */
Server.prototype._dtraceStartHandler = function _dtraceStartHandler(name, req) {
    var self = this;
 ...
 * @fires Request,Response#request
 */
Server.prototype._dtraceStopHandler = function _dtraceStopHandler(
    name,
    req,
    res
) {
...
Server.prototype._onRequest = function _onRequest(req, res) {
    ...
    // Dtrace run pre handlers
    self._dtraceStartHandler('pre', req);
    ...
    if (self.handleUncaughtExceptions) {
        ...
        handlerDomain.run(function run() {
            self.preChain(req, res, function preChainDone(err) {
                self._afterPre(err, req, res);
            });
        });
    } else {
 ...
 * @instance
 * @function _afterPre
 * @param  {Error|false|undefined} preErr - pre handler error
 * @param  {Request} req - request
 * @param  {Response} res - response
 * @returns {undefined} no return value
 */
Server.prototype._afterPre = function _afterPre(preErr, req, res) {
    var self = this;

    // DTrace pre done
    self._dtraceStopHandler('pre', req, res);

    // Handle error
    if (preErr) {
        self._afterRoute(preErr, req, res);
        return;
    }
    ...
    // DTrace support
    self._dtraceStartHandler('use', req);

    self.useChain(req, res, function useChainDone(useErr) {
        self._afterUse(useErr, req, res);
    });
};
 ...
 * @returns {undefined} no return value
 */
Server.prototype._afterUse = function _afterUse(err, req, res) {
    // DTrace
    this._dtraceStopHandler('use', req, res);

    // Handle error
    if (err) {
        this._afterRoute(err, req, res);
        return;
    }

    // Stop
    if (err === false) {
        this._afterRoute(null, req, res);
        return;
    }
...
/**
 * After route handlers found, before running them.
 * Warning: This method is only called when route is found.
 *
 * @private
 * @memberof Server
 * @instance
 * @function _afterRouteLookup
 * @param  {Request} req - request
 * @param  {Response} res - response
 * @returns {undefined} no return value
 */
Server.prototype._afterRouteLookup = function _afterRouteLookup(req, res) {
    this._dtraceStartHandler(req.route.name, req);
};
...
Server.prototype._afterRoute = function _afterRoute(err, req, res) {
    ...
    // DTrace
    if (route) {
        this._dtraceStopHandler(route.name, req, res);
    }

    // Call route by name
    if (_.isString(err)) {
        self.router.lookupByName(err, req, res);
        return;
    }
