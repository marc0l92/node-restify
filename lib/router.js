...
function Router(options) {
    ...
    this._afterRouteLookup = options._afterRouteLookup; // or set via server
...
Router.prototype.lookupByName = function lookupByName(name, req, res) {
    ...
    route.chain(req, res, function afterRouter(err) {
        self._afterRoute(err, req, res);
    });
};
...
Router.prototype.mount = function mount(opts, handlers) {
    ...
    self.findMyWay.on(route.method, route.path, function onRoute(
        ...
        self._afterRouteLookup(req, res);

        chain(req, res, function afterRouter(err) {
            self._afterRoute(err, req, res);
        });
    });
...
Router.prototype.getDebugInfo = function getDebugInfo() {
    return _.mapValues(this.mounts, function mapValues(route, routeName) {
        return {
            ...
            handlers: connect.extractHandlers(route.chain)
        };
    });
};
