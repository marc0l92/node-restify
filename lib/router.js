 ...
 * @returns  {String} url with qs
 */
Router.sanitizeUrl = function sanitizeUrl(originalUrl) {
    for (var i = 0, len = originalUrl.length; i < len; i++) {
        var charCode = originalUrl.charCodeAt(i);
        if (charCode === 63 || charCode === 35) {
            return originalUrl.slice(0, i);
        }
    }
    return originalUrl;
};
...
Router.prototype.lookup = function lookup(req, res, next) {
    ...
    var fmwRoute = self.findMyWay.find(req.method, Router.sanitizeUrl(req.url));
