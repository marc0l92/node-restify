...
function Server(options) {
    ...
    this.router.on('routed', this.emit.bind(this, 'routed'));
...
Server.prototype._runPre = function _runPre(req, res) {
    ...
    self.preChain.handle(req, res, function preChainDone(err) {
        self._afterPre(err, req, res);
    });
};
...
Server.prototype._runUse = function _runUse(req, res) {
    ...
    self.useChain.handle(req, res, function useChainDone(useErr) {
...
Server.prototype._runRoute = function _runRoute(req, res) {
    ...
    self.router.lookup(req, res, function _runRouteDone(routeErr) {
...
Server.prototype._afterRoute = function _afterRoute(err, req, res) {
    ...
    // Call route by name
    if (_.isString(err)) {
        self.router.lookupByName(err, req, res, function byName(routeErr) {
            self._afterRoute(routeErr, req, res);
        });
        return;
    }
